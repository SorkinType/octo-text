...
def analyze_text(input_file, output_file):
    ...
    def get_ngrams(n):
        ngrams = collections.Counter(zip(*[text[i:] for i in range(n)]))
        sorted_ngrams = sorted(ngrams.items(), key=lambda x: x[1], reverse=True)
        return ["".join(ngram) for ngram, _ in sorted_ngrams]
    ...
    with open(output_file, 'w', encoding='utf-8') as file:
        ...
        file.write(' '.join(bigrams[:10]) + '\n\n')

        file.write("Most common trigrams:\n")
        trigrams = get_ngrams(3)
        file.write(' '.join(trigrams[:10]) + '\n\n')
